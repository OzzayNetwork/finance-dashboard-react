...
const Bursars=()=>{
    ...
    const[blinkUserType,setBlinkUserType]=useState("Bursar")
    ...
    const profileNameFormatter=(cell,row)=>{
     ...
     //currnecy on the table
    const currencyFormatter=(cell,row)=>{
        var theAmount
        row?.userProfile?.blinkaccounts?.map((item) => {
            console.log(item);
            if (item.blinkersAccountType === "POCKECT_MONEY") {
                console.log("FDSHJGDSFHSDGSHD" + item.currentBalance);
                theAmount=item.currentBalance
            }
        })
        // console.log(row.userProfile.blinkaccounts.find((item) => {item.accountName === "POCKECT_MONEY"}))
        return (<span class="text-black">{StdFunctions.kenyaCurrency(theAmount)}</span>)
     }
     ...
     const capitalizeString=(cell,row)=>{
        return(
            <>
                <span class="text-capitalize">{cell?.toLowerCase()}</span>
            </>
        )
     }
    ...
    return(
        <>
            <Helmet>
                <title>Blink! Admin | All Bursars</title>
            </Helmet> 
            <div className="container-fluid">
                <div className="row">
                    <div className="col-12">
                        <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 className="mb-sm-0 font-size-18">All Bursars</h4>

                            <div className="page-title-right">
                                <ol className="breadcrumb m-0">
                                    <li className="breadcrumb-item"><Link to="/">Dashboards</Link></li>
                                    <li className="breadcrumb-item"><a href="#">Blink Users</a></li>
                                    <li className="breadcrumb-item active"><Link to="/">Blinkers</Link></li>
                                </ol>
                            </div>
